...
They give a **verbal** clue, spoken out loud and never typed into anything. The other
...
`lib/` is kept pure because it holds the real invariants, and it is testable in
Node without a DOM.
...
to the design's original values, so fidelity is enforced instead of trusted.
...
Moving labels radially inward makes this worse. At r=200 the overflow is
...
**86.5° is a measured value.** At that angle two-character labels (`C4`) clear the edge
by 2.8 units and three-character ones (`C#4`) overhang by 2.9,
...
legitimate, since it syncs an external system (the microphone), and the linter has no way to
...
cents, since a narrower span would draw a scale with no labels on it. **These two are
