...
what you hear and what the mic hears. No server, no accounts, and no audio leaves the
tab. Nothing persists across a reload except the default delay setting. Recordings live
in memory: this is an instrument, not a DAW project file.

## Layers

```
config.ts        every tunable in one object; tests derive expectations from it
...
the engine. That split lets every cancel/discard/graduation rule in the spec be a
...
hardware sample clock and cannot drift apart over minutes, which is the classic looper
failure.
...
The worklet is kept dumb: it moves samples and posts block RMS. Onset
...
second. So the hook extracts twice: a quick window (already written, since the track is
...
top, replacing instead of mixing. The original is silenced inside the bounds but stored
...
implementation does not have one: since every baked buffer is exactly one
...
`TIME_EPSILON` (1µs, far under a sample) matters in two places, both because
...
within that tolerance too: exact comparison deferred a transition landing on
...
under the new one, producing a flam on the most important click in the interaction, with the
...
Three details matter. The start is **backed off** by
...
tracks graduated. None of that is meaningful after a reload. `parseSnapshot`
...
shorter press is an ordinary save, and the arm delay protects that. Without it
...
the gesture's only feedback, and the bar *is* the timer. Painting
...
localStorage and survive by design: they are settings, not part of the loop, and
...
is recoverable by saving again, but it is a hole.
...
events. Where a key has no meaningful target at all, such as a bus that does not exist
or nothing left to delete, it returns `null` and the browser's own behavior is left
alone.
...
The arrows carry one trap. `selectTrack` **toggles**, so dispatching it with the
currently-selected id deselects instead of doing nothing, which is why ↑/↓
...
selectable tracks and do not, because a vertical list that jumps from bottom
...
`input`/`textarea`/`select`/contenteditable. That also stops the Enter that commits a
rename from starting the loop, since the keydown's target is still the field,
...
separate wetness knob. Reaching for `irSeconds` alone to get a longer reverb, which is
the obvious move, does nothing audible.
...
The metronome still bypasses the master fader, mute and reverb, since muting the master
must not silence a count-in, but it now routes **into the limiter** instead of straight to
...
`useTrackDrag` measures rows in a `useLayoutEffect` keyed on the drag *starting*, that
is, after the commit in which `dragActive` collapses every row. That ordering is required:
...
visibly swims. `preventDefault` is called only once a drag has started, so
until then a touch swipe still scrolls the list; touch additionally requires a
long-press to arm, which keeps scrolling and reordering from fighting over the
same gesture.
...
required; reverting either brings back a `react-hooks/refs` build error.
...
- **Not** `@/shared/audio`'s `useMicrophone`: it closes its private `AudioContext` on
  stop and cannot host a worklet or playback graph. The engine owns its own context,
...
| `lib/shortcuts.test.ts` | every key mapping, multiplier and bus wrap-around at both ends, arrow track-stepping (clamped, locked tracks skipped, the selectTrack-toggle trap), and the delete rule covering selection, bottom-most, and stale selections ignored |
...
the handover into recording. IndexedDB has no coverage at all. `storage.ts` is kept
decision-free so that everything worth pinning lives in `snapshot.ts`.
